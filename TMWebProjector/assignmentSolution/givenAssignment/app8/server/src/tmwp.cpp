...
void Request::extensionTPL(const char *file,int clientSocketDescriptor)
{
int i,j;
int count=0;
...
{
count=1;
if(str1[i+1]!='\0'&&str1[i+1]=='$')count=2;
if(str1[i+2]!='\0'&&str1[i+2]=='$')count=3;
if(str1[i+3]!='\0'&&str1[i+3]=='{')count=4;
}
if((count==4)||(count==3&&str1[i+1]=='{')||(count==2&&str1[i+1]=='$'&&str1[i+2]=='{')||(count==1&&str1[i+1]=='$'&&str1[i+2]=='$'&&str1[i+3]=='{'))
{
str2[j]='\0';
send(clientSocketDescriptor,str2,strlen(str2)+1,0);
i+=(count);
for(j=0;str1[i]!='\0'&&str1[i]!='}';j++,i++)
{
str2[j]=str1[i];
}
str2[j]='\0';
...
iter=keyValues.find(string(str2));
if(iter==keyValues.end())kkk="";
else kkk=iter->second;
send(clientSocketDescriptor,kkk.c_str(),strlen(kkk.c_str())+1,0);
j=-1;
count=0;
}
}//for loop ends
...
send(clientSocketDescriptor,str1,strlen(str1)+1,0);
...
{
string extension=getExtension(request->resource);
if(extension=="tpl")
{
printf("***************\n");
sprintf(header,"HTTP/1.1 200 OK\nContent-Type:text/html\n\n");
send(clientSocketDescriptor,header,strlen(header)+1,0);
request->extensionTPL(request->resource,clientSocketDescriptor);
}
else
{
fseek(f,0,2);
...
{
// the resource part against request and reprocess the request
