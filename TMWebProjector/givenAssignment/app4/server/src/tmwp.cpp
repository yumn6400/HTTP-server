...
response.write("</head>");
response.write("<body>");
response.write("Sending 404 page");
response.write("</body>");
response.write("</html>");
}
closesocket(request->clientSocketDescriptor);
}

map<const char *,int> Request::requestValue;
...
{
map<const char *,int>::iterator k;
k=this->requestValue.find(data);
if((k==this->requestValue.end()))
{
return 0;
}
else
