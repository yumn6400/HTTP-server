...
{
this->request=request;
int clientSocketDescriptor=this->request->clientSocketDescriptor;
char header[501];
...
{
int clientSocketDescriptor=this->request->clientSocketDescriptor;
send(clientSocketDescriptor,data,strlen(data)+1,0);
}
void Response::close()
{
closesocket(this->request->clientSocketDescriptor);
}
