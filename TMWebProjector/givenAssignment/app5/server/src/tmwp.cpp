...
string getExtension(const char *str)
{
int i;
for(i=0;str[i]!='\0';i++)
{
...
char isClientSideResource(char *resources)
{
int i;
for(i=0;resources[i]!='\0'&&resources[i]!='.';i++);
if(resources[i]=='\0')return 'N';
return 'Y';
}
...
void Request::sendSCTSubjectName(const char *data)
{
int value;
char funcValue[11];
if(extensionEquals(data,"{physics}"))
{
value=getInt("physics");
sprintf(funcValue,"%d",value);
send(request->clientSocketDescriptor,funcValue,strlen(funcValue),0);
}
else if(extensionEquals(data,"{chemistry}"))
{
value=getInt("chemistry");
sprintf(funcValue,"%d",value);
send(request->clientSocketDescriptor,funcValue,strlen(funcValue),0);
}
else if(extensionEquals(data,"{mathematics}"))
{
value=getInt("mathematics");
sprintf(funcValue,"%d",value);
send(request->clientSocketDescriptor,funcValue,strlen(funcValue),0);
}
else if(extensionEquals(data,"{english}"))
{
value=getInt("english");
sprintf(funcValue,"%d",value);
send(request->clientSocketDescriptor,funcValue,strlen(funcValue),0);
}
else if(extensionEquals(data,"{hindi}"))
{
value=getInt("hindi");
sprintf(funcValue,"%d",value);
send(request->clientSocketDescriptor,funcValue,strlen(funcValue),0);
...
void Request::forwardToExtensionSCT(const char *str)
{
FILE *f;
int i,rc,len,j;
...
data[j]='\0';
len2=ftell(f);
sendSCTSubjectName(data);
}
arr[0]=a;
arr[1]='\0';
if(a!='$')send(request->clientSocketDescriptor,arr,1,0);
}
fclose(f);
}
void Request::forward(const char *data)
{
char responseBuffer[1025];
char str[101];
int i,rc,len;
...
if(data[0]=='/')ii=1;
strcpy(str,data+ii);
if(isClientSideResource(str)=='Y')
{
FILE *f;
f=fopen(str,"rb");
if(f==NULL)
{
sprintf(responseBuffer,"<!DOCTYPE html><html lang='en'><head><title>Whatever</title></head><body><h1>Resource /%s not found</h1></body></html>",str);
send(request->clientSocketDescriptor,responseBuffer,strlen(responseBuffer)+1,0);
}
else
{
...
send(request->clientSocketDescriptor,responseBuffer,rc,0);
i=i+rc;
}
}
}
...
for(k=t->dataCollection.begin();k!=t->dataCollection.end();k++)
{
if(extensionEquals(k->first,str))
{
found=1;
break;
}
...
{
sprintf(responseBuffer,"<!DOCTYPE html><html lang='en'><head><title>Whatever</title></head><body><h1>Resource /%s not found due to some reason </h1></body></html>",request->resource);
send(request->clientSocketDescriptor,responseBuffer,strlen(responseBuffer)+1,0);
}
}
}
...
void Request::setInt(const char * data,int value)
{
this->requestValue.insert(pair<const char *,int>(data,value));
}
int Request::getInt(const char *data)
{
int found=0;
map<const char *,int>::iterator i;
for(i=requestValue.begin();i!=requestValue.end();i++)
{
if(extensionEquals(i->first,data))
{
found=1;
break;
}
}
if(found==1)return i->second;
else return 0;
}
...
map<char *,void(*)(Request &,Response &)>TMWebProjector::dataCollection;
